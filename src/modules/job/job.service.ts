...
export class JobService {
  ...
  async createJob(
    ...
    if (!user.providerServiceAvailability)
      throw new NotFoundException(
        'Your account is currently unavailable due to administrative restrictions. Please contact support for more information.',
      );
  ...
  async deleteJob(userId: string, jobId: string) {
    ...
    if (userId !== job.userId)
      throw new NotFoundException('You are not job owner.');
  ...
  async globalSearch(search: string) {
    ...
    (
      SELECT 
        ...
        j.slug,
        j.thumbnail,
        j."categoryId" as "extraId"
      FROM "jobs" j
      WHERE 
        j."deletedAt" IS NULL
        AND j."isDelete" = false
        AND (
          j.id::text ILIKE $1 OR
          j."categoryId"::text ILIKE $1 OR
          j.title ILIKE $1 OR
    ...
    (
      SELECT 
        b.id,
        b."serviceName" as title,
        b."serviceDescription" as description,
        b."createdAt",
        'booking' as "type",
        NULL as "slug",
        NULL as thumbnail,
      ...
      FROM "bookings" b
      WHERE 
        b.id::text ILIKE $1 OR
        b."jobId"::text ILIKE $1 OR
        b."clientId"::text ILIKE $1 OR
        b."providerId"::text ILIKE $1 OR
        b."bookingNumber" ILIKE $1 OR
        b."serviceName" ILIKE $1 OR
        b."serviceDescription" ILIKE $1
    )
