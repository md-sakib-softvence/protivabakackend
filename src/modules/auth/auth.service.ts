...
export class AuthService {
  ...
  async forgotPassword(dto: ForgotPasswordDto) {
    ...
    return {
      message: 'We have sent an OTP to your email. Please check your email.',
    };
  }
  ...
  async createAdminUser(userId: string, dto: AdminUserDto) {
    ...
    if (dto.isViewBooking || dto.isManageBooking || dto.isExportBooking) {
      await this.prisma.notification.create({
        data: {
          ...
          message:
            'You have been granted permissions to view, manage, and export bookings.',
          type: 'ADMMIN_PERMISSION_GRANTED',
        },
      });
    }
    ...
    if (dto.isViewProvider || dto.isManageProvider) {
      await this.prisma.notification.create({
        data: {
          ...
          message:
            'You have been granted permissions to view and manage providers.',
          type: 'ADMMIN_PERMISSION_GRANTED',
        },
      });
    }
    ...
    if (dto.isViewUser || dto.isManageUser) {
      await this.prisma.notification.create({
        data: {
          ...
          message:
            'You have been granted permissions to view and manage users.',
          type: 'ADMMIN_PERMISSION_GRANTED',
        },
      });
    }
    ...
    if (dto.isViewCategory || dto.isManageCategory) {
      await this.prisma.notification.create({
        data: {
          ...
          message:
            'You have been granted permissions to view and manage categories.',
          type: 'ADMMIN_PERMISSION_GRANTED',
        },
      });
    }
    ...
    if (dto.isViewWithdrawal || dto.isManageWithdrawal) {
      await this.prisma.notification.create({
        data: {
          ...
          message:
            'You have been granted permissions to view and manage withdrawals.',
          type: 'ADMMIN_PERMISSION_GRANTED',
        },
      });
    }
    ...
    if (
      ...
      !dto.isManageUser &&
      !dto.isViewCategory &&
      !dto.isManageCategory &&
      !dto.isViewTransaction &&
      !dto.isViewWithdrawal &&
      !dto.isManageWithdrawal
    ) {
      await this.prisma.notification.create({
        data: {
          userId: adminUser.id,
          ...
          message:
            'You have been created as a sub-admin but no permissions have been granted yet. Please contact a super-admin to assign you permissions.',
          type: 'ADMMIN_PERMISSION_GRANTED',
        },
      });
    }
  ...
  async updateAdminUserPermissions(userId: string, dto: UpdatePermissionDto) {
    ...
    if (requestingUser.role !== UserRole.SUPER_ADMIN) {
      throw new UnauthorizedException(
        'You are not a super-admin. Only super-admins can update permissions',
      );
    }
  ...
  async updateUserProfile(userId: string, dto: UpdateProfileDto) {
    ...
    if (dto.fildName === 'email' || dto.fildName === 'phone') {
      const where =
        dto.fildName === 'email' ? { email: dto.value } : { phone: dto.value };
      ...
      if (existing && existing.id !== userId) {
        throw new BadRequestException(`${dto.fildName} already exists`);
      }
    }
    ...
    await this.sentNotification(
      user.id,
      'Profile Updated',
      `Your profile has been updated successfully. ${dto.fildName} is now ${dto.value}.`,
    );
  ...
  async updateUserProfilePicture(userId: string, avater: Express.Multer.File) {
    ...
    const avaterUp: any = await this.cloudinary.uploadImageFromBuffer(
      avater.buffer,
      'avater',
      `${Date.now()}-${avater.originalname}`,
    );
  ...
  async addNewProvider(
    ...
    if (checkPhone)
      throw new BadRequestException('Already use this phone number');
    ...
    const nidImageUp: any = await this.cloudinary.uploadImageFromBuffer(
      nidImage.buffer,
      'nidImage',
      `${Date.now()}-${nidImage.originalname}`,
    );
    const nidBackImageUp: any = await this.cloudinary.uploadImageFromBuffer(
      nidBackImage.buffer,
      'nidBackImage',
      `${Date.now()}-${nidBackImage.originalname}`,
    );
    ...
    await this.sentNotification(
      ...
      `Congratulation, You are a new provider in kajBD family.`,
    );
